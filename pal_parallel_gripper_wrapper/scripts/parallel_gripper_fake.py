...
class WrappedFJTA(object):
    ...
    def __init__(self):
        ...
        if not self.real_joint_names:
            rospy.logerr("No real joint names given in param: ~real_joint_names")
            exit(0)
        self.gripper_joint_name = rospy.get_param("~gripper_joint_name", None)
        if not self.gripper_joint_name:
            rospy.logerr("No gripper joint name given in param: ~gripper_joint_name")
            exit(0)
    ...
    def execute_cb(self, goal):
        ...
        # We ignore path_tolerance and goal_tolerance... does not make sense in a gripper
    ...
    def state_msg(self, ref_state, type_msg):
        ...
        st.desired.positions.extend(ref_state.desired.positions[:1] * 2.0)
        ...
        st.actual.positions.extend(ref_state.actual.positions[:1] * 2.0)
        ...
        st.error.positions.extend(ref_state.error.positions[:1] * 2.0)
